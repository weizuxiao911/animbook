 ...
 *   - WebSocket 连 /ai/pty/{id}/connect → onData 回调终端
 *   - onMessage(id, json) → WebSocket 发送 input (含 resize 指令)
 *   - disposeById → pty.remove
 *
 * cwd 通过 src/commands/fs.ts 的 getWorkspaceDirSync() 拿 (启动期已从 /ai/path 拉到).
 */
...
export class OpenCodePtyService implements ITerminalServiceClient {
  ...
  private get client(): any {
    return (window as any).__ANIMBOOK_OPENCODE__ || null;
  }
  ...
  /** wsUrl: 同源 /ai/pty/... (webpack dev 代理到 opencode, 生产由反向代理承接) */
