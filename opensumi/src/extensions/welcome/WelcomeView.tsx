...
import { getWorkspaceDirSync } from '../../_legacy/commands/fs';

/**
 * WelcomeView — animbook 主区欢迎页
 *
...
export const WelcomeView: React.FC<{ resource?: any }> = () => {
  ...
  const workspaceDir = getWorkspaceDirSync() || '';
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const f of Array.from(files)) {
      try {
        ...
        const idePath = `/${safe}`;
        await fsApi.write(idePath, text);
        results.push(safe);
      } catch (err) {
  ...
  return (
    <div className="ab-welcome">
      ...
      <div className="ab-welcome__inner">
        ...
        <h1 className="ab-welcome__title">animbook</h1>
        <p className="ab-welcome__subtitle">可交互式阅读器</p>
