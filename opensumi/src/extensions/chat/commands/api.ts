...
import { getOpencodeClient, isOpencodeReady, waitForOpencodeReady } from '../../../_legacy/commands/sandbox';
...
export async function aiListAgents(): Promise<any[]> {
  ...
  const dir = typeof window !== 'undefined' && window.location?.pathname
    ...
    : '.';
  const url = `/api/agent?directory=${encodeURIComponent(dir)}`;
...
export async function aiListSkills(): Promise<SkillInfo[]> {
  ...
  const dir = typeof window !== 'undefined' && window.location?.pathname
    ? window.location.pathname
    : '.';
  const url = `/api/skill?directory=${encodeURIComponent(dir)}`;
  const res = await fetch(url, { headers: { Accept: 'application/json' } });
...
export async function aiListCommands(): Promise<CommandInfo[]> {
  ...
  const dir = typeof window !== 'undefined' && window.location?.pathname
    ? window.location.pathname
    : '.';
  const url = `/api/command?directory=${encodeURIComponent(dir)}`;
  const res = await fetch(url, { headers: { Accept: 'application/json' } });
