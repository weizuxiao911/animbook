 ...
 *   animbook = opencode AI 客户端 + 浏览器 IDE. 左侧入口不做"团队/空间分组",
 *   而是 animbook 自身能力的快速跳转 + 真实 AI 会话历史.
 *
 *   - 顶部 brand + 当前工作空间 (cwd)
...
export const SessionsView: React.FC<SessionsViewProps> = (props) => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('animbook:opencode-ready', onReady);
    const onAiRefresh = () => { void refresh(); };
    window.addEventListener('animbook:ai-sessions-changed', onAiRefresh);
    return () => {
      clearInterval(t);
      window.removeEventListener('animbook:opencode-ready', onReady);
      window.removeEventListener('animbook:ai-sessions-changed', onAiRefresh);
    };
  }, [refresh]);
  ...
  const onNewSession = useCallback(async () => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('animbook:ai-sessions-changed'));
      selectSession(id);
    } catch (e) {
      setError(String((e as any)?.message || e));
    } finally {
